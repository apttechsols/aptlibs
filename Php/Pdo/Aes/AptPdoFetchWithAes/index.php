	...
	*/
	...
	if(!defined('EPASS') or EPASS != 'UwRxVTHAN6IeR3IZpLD8Tg8u2twZW56mN'){
		function AptPdoFetchWithAes($Data=array()){
			return ['status'=>'Error','data'=>'Autenication failed [Apt Fetch Data With Aes]','code'=>401]; die(); exit();
		}
	}else{
		function AptPdoFetchWithAes($Data=array()){
			...
			if(!array_key_exists("DbCon",$Data) or !array_key_exists("TbName",$Data) or !array_key_exists("EPass",$Data) or $DbCon == '' or $TbName == '' or $EPass == ''  or $DefaultCheckFor == '' or $DefaultCheckType == '' or $FetchRowNo == '' or ($AcceptNullCondtion != False and $AcceptNullCondtion != true)){
			...
			if($FetchData != ''){
				...
				foreach ($FetchDataArray as $value) {
					...
					if(preg_replace("/[^A-Za-z0-9_,:]/","",$value) != "" && preg_replace("/[^A-Za-z0-9_,:]/","",$value) == $value){
						...
						$TmpFetchDataFormat = "AES_DECRYPT(".$value.", :EPass) AS ".$value;
						if($TmpArray[1] != ''){
